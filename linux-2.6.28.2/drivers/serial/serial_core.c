...
#define SYS_UART_RXD_PCTRL		__REG (SYS_PHYS + 0x148)
#define SYS_UART_RXD_PULLUP_ENABLE	0x00
#define SYS_UART_RXD_PULLUP_DISABLE	0x02

extern void register_rx_irq(void);
...
int uart_suspend_port(struct uart_driver *drv, struct uart_port *port)
{
	...
	if(1) {
	// Enable RxD pin pull-up
		register_rx_irq();
		SYS_UART_RXD_PCTRL = SYS_UART_RXD_PULLUP_ENABLE;
		enable_irq_wake(IRQ_BT_RECEIVE_INT);
...
//	if (!port->suspended && device_may_wakeup(tty_dev)) {
	if(!port->suspended){
		//Disable RxD pin pull-up
		SYS_UART_RXD_PCTRL = SYS_UART_RXD_PULLUP_DISABLE;
		disable_irq_wake(IRQ_BT_RECEIVE_INT);
