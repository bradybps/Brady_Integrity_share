...
#define DEFAULT_HEARTBEAT MAX_HEARTBEAT
#define WDT_IN_USE        0
...
#define TOGGLE_MR1   	(3 << 6)
#define DO_NOTHING_MR1	(0 << 6)
...
static void lpc313x_wdt_start(struct lpc313x_wdt * wdt)
{
	...
	writel(TOGGLE_MR1, base + LPC313x_WDT_EMR);
...
static int lpc313x_wdt_probe(struct platform_device *pdev)
{
	...
	wdt->boot_status = (readl((void __iomem *) io_p2v(CGU_SB_PHYS)) & 0x1) ?
		WDIOF_CARDRESET : 0;
	lpc313x_wdt_stop(wdt);
	lpc313x_wdt_start(wdt); /*disable wdt from userspace*/
