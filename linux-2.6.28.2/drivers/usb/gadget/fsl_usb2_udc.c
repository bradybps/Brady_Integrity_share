...
static int fsl_ep_disable(struct usb_ep *_ep)
{
	...
	ep->ep.maxpacket = (unsigned short) ~0;  /*****pd bug fix***/
	ep->desc = NULL;
	ep->stopped = 1;
	spin_unlock_irqrestore(&udc->lock, flags);
...
static int __init fsl_udc_probe(struct platform_device *pdev)
{
	...
	ret = fsl_udc_clk_init(pdev, dr_regs);
	if (ret < 0)
...
{
	if(udc_controller->driver) {
		if (udc_controller->driver->suspend)
			udc_controller->driver->suspend(&udc_controller->gadget);
	}
	lpc_udc_suspend(pdev);
	return 0;
}
...
static int fsl_udc_resume(struct platform_device *pdev)
{
	...
	if(udc_controller->driver) {
		if (udc_controller->driver->resume)
			udc_controller->driver->resume(&udc_controller->gadget);
	}
	return 0;
}
