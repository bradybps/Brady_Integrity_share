...
static struct nand_bbt_descr bps_bbt_main_descr = {
	.options = NAND_BBT_LASTBLOCK | NAND_BBT_CREATE | NAND_BBT_WRITE
		| NAND_BBT_2BIT |  NAND_BBT_PERCHIP,
	.offs = 0,
	.len = 4,
	.veroffs = 1,
	.maxblocks = 4,
...
static struct nand_bbt_descr bps_bbt_mirror_descr = {
	.options = NAND_BBT_LASTBLOCK | NAND_BBT_CREATE | NAND_BBT_WRITE
		| NAND_BBT_2BIT |  NAND_BBT_PERCHIP,
	.offs = 0,
	.len = 4,
...
static struct nand_bbt_descr bps_bbt_ff_descr = {
	.options = 0,
	.offs = 5,
	.len = 1,
	.pattern = scan_ff_pattern
...
#else
	#define BPS_NAND_TIMING1 (0x00001001)
	#define BPS_NAND_TIMING2 (0x00102523)

	nand_writel(TIMING1,BPS_NAND_TIMING1);
	nand_writel(TIMING2,BPS_NAND_TIMING2);
#endif
}
...
#ifdef CONFIG_MACH_BPS313XV1
/*
 *  As a fix to the BUG in NXP's lpc313x_nand_read_oob_syndrome function
 */
static int bps_nand_read_oob_syndrome(struct mtd_info *mtd, struct nand_chip *chip,
			     int page, int sndcmd)
{
	if (sndcmd) {
...
#ifdef CONFIG_MACH_BPS313XV1
/*
 *  As a fix to the BUG in NXP's lpc313x_nand_write_oob_syndrome function
 */
static int bps_nand_write_oob_syndrome(struct mtd_info *mtd, struct nand_chip *chip,
			      int page)
{
	int status = 0;
...
#ifdef CONFIG_MACH_BPS313XV1
	chip->ecc.read_oob = bps_nand_read_oob_syndrome;
	chip->ecc.write_oob = bps_nand_write_oob_syndrome;
#else
...
#ifdef CONFIG_MACH_BPS313XV1
		chip->bbt_td = &bps_bbt_main_descr;
		chip->bbt_md = &bps_bbt_mirror_descr;
		chip->badblock_pattern = &bps_bbt_ff_descr;
#endif
	}
#ifdef HUGE_BLOCK_SUPPORT
