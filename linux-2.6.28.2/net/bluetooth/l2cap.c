...
static inline int l2cap_send_cmd(struct l2cap_conn *conn, u8 ident, u8 code, u16 len, void *data)
{
	...
	BT_DBG("conn 0x%x", conn);
	BT_DBG("ident 0x%x", ident);
	BT_DBG("len %d", len);
...
static int l2cap_parse_conf_req(struct sock *sk, void *data)
{
	...
	if (result == L2CAP_CONF_SUCCESS) {
		...
		if (rfc.mode == L2CAP_MODE_BASIC) {
			/* Bluetooth Certification Change:
			 * According to L2CAP Specification, the Minimum MTU Supported 
			 * should be 48,but now we are checking if the configuration is requesting
			 * less than the OUT MTU, which is also by default 672,So To Address
			 * thi issue, we need to change the IN MTU to 48, or remove the check 
			 * here.
			 *
			 * This Change causes the Connection to be alive as long as there is no
			 * Disconnect request sent by either of 2. But the Pairing is successful.
			 *
			 */
			if (mtu < pi->omtu) 
				result = L2CAP_CONF_UNACCEPT;
				 ...
				 pi->omtu = mtu; 
				 BT_DBG("mtu set %d : %d : %d", mtu, pi->omtu, pi->imtu);
				 /* Bluetooth Certification Changes */
				 pi->imtu = mtu;
				 BT_DBG("mtu set %d : %d : %d", mtu, pi->omtu, pi->imtu);
				 /* Bluetooth Certification Changes */
...
static inline int l2cap_config_req(struct l2cap_conn *conn, struct l2cap_cmd_hdr *cmd, u16 cmd_len, u8 *data)
{
	...
	if (!(l2cap_pi(sk)->conf_state & L2CAP_CONF_REQ_SENT)) {
		...
		l2cap_pi(sk)->conf_state |= L2CAP_CONF_REQ_SENT; 
		/* Bluetooth Certification Changes */
		BT_DBG("Bluetooth Sending again!!!!");
		l2cap_send_cmd(conn, l2cap_get_ident(conn), L2CAP_CONF_REQ,
...
static inline void l2cap_sig_channel(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	while (len >= L2CAP_CMD_HDR_SIZE) {
		...
		default:
			BT_DBG("Unknown signaling command 0x%2.2x", cmd.code);
			err = -EINVAL;  
			//err = 1;//Make sure L2CAP_COMMAND_REJ is sent.
