...
static int lpc313x_enter_sleep(u32 standby)
{
	...
	if (standby == 0) {
		...
		/* LPC313x USB wakeup fix:
		 * The InterruptController needs to be activated to enable wake
		 * up from USB resume interrupt
		 */
	...
	/* 
	 * switch off the leds to signify that the system entered 
	 * sleep state
	 */
