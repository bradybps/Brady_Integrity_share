...
int __init lpc313x_init(void)
{
	...
	/* I2S TX0 WS, DATA */
	LPC313X_GPIO_DRV_IP(IOCONF_EBI_I2STX_0, 0x60);

	/* I2S TX0 BCK */
	LPC313X_GPIO_DRV_IP(IOCONF_EBI_MCI, 0x80);
#endif

#if defined (CONFIG_SND_I2S_TX1_MASTER)
	/* I2S TX1 BCK, WS, DATA */
	LPC313X_GPIO_DRV_IP(IOCONF_I2STX_1, 0x7);
#endif

#if defined (CONFIG_SND_I2S_RX0_MASTER) | defined (CONFIG_SND_I2S_RX0_SLAVE)
	/* I2S RX0 BCK, WS, DATA */
	LPC313X_GPIO_DRV_IP(IOCONF_I2SRX_0, 0x7);
#endif
#if defined (CONFIG_SND_I2S_RX1_MASTER) | defined (CONFIG_SND_I2S_RX1_SLAVE)
	/* I2S RX1 BCK, WS, DATA */
	LPC313X_GPIO_DRV_IP(IOCONF_I2SRX_1, 0x7);
#endif
	/* AUDIO CODEC CLOCK (256FS) */
	LPC313X_GPIO_DRV_IP(IOCONF_I2STX_1, 0x8);
#endif
...
static int __init lpc313x_init_console(void)
{
	...
	if (div != 0)  {
		up.uartclk = (36000000 * mul) / (mul + div); 
	} 
	up.regshift	= 2;
	up.iotype	= UPIO_MEM;
